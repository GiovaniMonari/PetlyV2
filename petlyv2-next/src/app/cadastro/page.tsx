...
import { Mail, Lock, User, MapPin, Eye, EyeOff, ArrowRight, ArrowLeft } from 'lucide-react';
...
export default function CadastroPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!acceptedPolicies) {
      setPolicyError('Você deve aceitar os Termos de Uso e a Política de Privacidade para criar sua conta.');
      return;
    }
        ...
        <div className="mb-8 flex flex-col items-center">
          <Link href="/" className="inline-flex items-center gap-2 mb-6 group">
            <BrandLogo
              size="lg"
              priority
              imageClassName={`rounded-2xl transition-all duration-500 ${
                accountType === 'tutor' ? 'group-hover:ring-[#06A77D]/40' : 'group-hover:ring-[#FF6B35]/40'
              }`}
            />
          </Link>
            ...
            <div className="flex flex-col gap-3 text-sm text-gray-300 mt-4">
              <label className="inline-flex items-start gap-3">
                <input
                  ...
                  className="mt-1 h-4 w-4 rounded border-white/20 bg-black/70 text-[#FF6B35] focus:ring-[#FF6B35]"
                />
                <span className="leading-[1.5]">
                  Li e concordo com os{' '}
                  <Link href="/termos" className="font-semibold text-[#FF6B35] hover:text-[#E55A2B]">
                    Termos de Uso
                  </Link>{' '}
                  e a{' '}
                  <Link href="/privacidade" className="font-semibold text-[#FF6B35] hover:text-[#E55A2B]">
                    Política de Privacidade
                  </Link>
                  .
                </span>
              </label>
              {policyError && <p className="text-xs text-red-500 font-medium ml-7">{policyError}</p>}
            </div>

            <button
              type="submit"
              disabled={isLoading || !acceptedPolicies}
              className={`w-full relative group overflow-hidden ${primaryBg} text-white font-semibold rounded-xl py-3 px-4 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:hover:scale-100 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-6`}
            >
